...
public class Enemy : MonoBehaviour, IDamageable
{
	...
	private void FixedUpdate()
	{
		if (!isIdle)
		{
			...
			Vector3 targetVelocity = new Vector2(directionMod * 2.5f, CurrentVelocity().y);
			...
			var direction = new Vector2(transform.position.x + 1 * directionMod, transform.position.y);
			var wallCheck = Physics2D.Raycast(transform.position, Vector2.right, 5);
			...
			if (wallCheck.transform.gameObject.layer == LayerMask.NameToLayer("Default"))
			{
				print("Near a wall...");
				StartCoroutine(StateStay(0.05f));
			}
		}
	}
